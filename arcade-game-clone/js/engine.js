...
var Engine = (function (global) {
    ...
    canvas.width = 505;
    canvas.height = 606;
    canvas.style.width = 505 + 'px';
    canvas.style.height = 606 + 'px';
    ...
    function main() {
         ...
         * 来调用这个函数
         */
        win.animationID = win.requestAnimationFrame(main);
        /* 由于cancelAnimationFrame要在requestAnimationFrame后执行，所以冲突检测
         * 函数放在最后。
         */
        enemyCollisions();
        gemCollisions();
    ...
    function enemyCollisions() {
        allEnemies.forEach(function (enemy) {
            if (enemy.y === player.y && enemy.x + 70 >= player.x && enemy.x <= player.x + 67) {
                stop();
                var endModal = document.getElementById("endModal");
                endModal.setAttribute("style", "visibility:show");
    ...
    function stop() {
        if (win.animationID) {
            win.cancelAnimationFrame(win.animationID);
        }
        if (win.timeoutID) {
    ...
    function gemCollisions() {
        gems.forEach(function (gem) {
            if (gem.x === player.x + 15 && gem.y === player.y + 32 && !gem.isCollected) {
    ...
    /* 在右上角绘制待收集的宝石 */
    function renderCollectedGems() {
        var img;
        for (i = 0; i < 3; i++) {
            if (gems[i].isCollected) {
                img = Resources.get(gems[i].sprite);
            } else {
                img = Resources.get(gems[i].blurSprite);
            }
            ctx.drawImage(img, 400 + i * 40, 0, img.width * 0.25, img.height * 0.25);
        }
    }
    ...
    /* 判断是否通关，达成通关条件后执行reset方法，并继承上一局的游戏分数 */
    function success() {
        if (player.y === -20 && gemCount === 3) {
            stop();
            var score = player.score;
            win.timeoutID = setTimeout(function () {
                reset();
                player.score = score;
            }, 500);
        }
    ...
    Resources.load([
        ...
        'images/Gem Blue-blur.png',
        'images/Gem Green-blur.png',
        'images/Gem Orange-blur.png',
     ...
     */
    global.ctx = ctx;
    /* 把reset函数绑定到 global 全局变量上，让外部可以访问 */
    global.reset = reset;
})(this);